...
namespace Framework.Persistence.Fields {
    ...
    /// <summary>
    /// Wrapper class for a field of type ushort to be stored in
    /// persistence mechanism.
    /// </summary>
    public sealed class PersistentFieldForUShort : PersistentFieldForStruct<ushort> {
        ...
        public PersistentFieldForUShort(string key)
            : base(key) {
            // nothing to do
        }
        ...
        public PersistentFieldForUShort(string key, ushort value)
            : this(key) {
            this.Value = value;
        }
        ...
        internal override void LoadValueFromDbDataReader(DbDataReader dataReader, int ordinal) {
            if (dataReader.IsDBNull(ordinal)) {
                this.valueFromDbDataReader = default(ushort);
            } else {
        ...
        public override bool TrySetValueAsString(string value) {
            ...
            if (string.IsNullOrEmpty(value)) {
                this.Value = default(ushort);
                success = true;
            } else {
                success = ushort.TryParse(value, NumberStyles.Any, CultureInfo.InvariantCulture, out ushort parsedValue);
