...
namespace Framework.Persistence.Fields {
    ...
    /// <summary>
    /// List of objects of type DateTime that can be stored in
    /// persistence mechanism.
    /// </summary>
    public sealed class PersistentFieldForDateTimeCollection : PersistentFieldForCollection<DateTime> {
        ...
        /// </summary>
        private List<DateTime> valuesFromDbDataReader;

        /// <summary>
        /// Instantiates a new instance.
        /// </summary>
        /// <param name="key">name of persistent field</param>
        public PersistentFieldForDateTimeCollection(string key)
            : base(key) {
            // nothing to do
        }
        ...
        protected override PersistentFieldForElement<DateTime> GetNewPersistentFieldForElement() {
            return new PersistentFieldForDateTime(this.Key);
        }
        ...
        public override IEnumerable<string> GetValuesAsString() {
            foreach (var dateTimeValue in this) {
                yield return UtcDateTime.FormatAsIso8601Value(dateTimeValue, DateTimeType.DateAndTime);
            }
        }
        ...
        internal override void InitializeTemporaryCollectionForDbDataReader() {
            this.valuesFromDbDataReader = new List<DateTime>();
            return;
        }
        ...
        internal override void LoadValueFromDbDataReader(DbDataReader dataReader) {
            this.valuesFromDbDataReader.Add(new DateTime(dataReader.GetInt64(0), DateTimeKind.Utc));
            return;
        }
        ...
        public override bool TryAddString(string item) {
            ...
            if (string.IsNullOrEmpty(item)) {
                ...
                success = UtcDateTime.TryParse(item, out DateTime parsedItem);
                if (success) {
                    this.Add(parsedItem);
                }
            }
