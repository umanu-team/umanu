...
namespace Framework.Persistence.Fields {
    ...
    /// <summary>
    /// List of objects of type decimal that can be stored in
    /// persistence mechanism.
    /// </summary>
    public sealed class PersistentFieldForDecimalCollection : PersistentFieldForCollection<decimal> {
        ...
        /// </summary>
        private List<decimal> valuesFromDbDataReader;

        /// <summary>
        /// Instantiates a new instance.
        /// </summary>
        /// <param name="key">name of persistent field</param>
        public PersistentFieldForDecimalCollection(string key)
            : base(key) {
            // nothing to do
        }
        ...
        protected override PersistentFieldForElement<decimal> GetNewPersistentFieldForElement() {
            return new PersistentFieldForDecimal(this.Key);
        }
        ...
        public override IEnumerable<string> GetValuesAsString() {
            foreach (var decimalValue in this) {
        ...
        internal override void InitializeTemporaryCollectionForDbDataReader() {
            this.valuesFromDbDataReader = new List<decimal>();
            return;
        }
        ...
        internal override void LoadValueFromDbDataReader(DbDataReader dataReader) {
            if (this.ParentPersistentObject.ParentPersistentContainer.ParentPersistenceMechanism.HasNativeSupportForDecimal) {
                this.valuesFromDbDataReader.Add(dataReader.GetDecimal(0));
            } else {
                this.valuesFromDbDataReader.Add(Convert.ToDecimal(dataReader.GetValue(0), CultureInfo.InvariantCulture.NumberFormat));
            }
            return;
        }
        ...
        public override bool TryAddString(string item) {
            bool success = decimal.TryParse(item, NumberStyles.Any, CultureInfo.InvariantCulture, out decimal parsedItem);
            this.Add(parsedItem);
            return success;
        }
