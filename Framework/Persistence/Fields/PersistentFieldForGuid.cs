...
namespace Framework.Persistence.Fields {
    ...
    /// <summary>
    /// Wrapper class for a field of type Guid to be stored in
    /// persistence mechanism.
    /// </summary>
    public sealed class PersistentFieldForGuid : PersistentFieldForStruct<Guid> {
        ...
        public PersistentFieldForGuid(string key)
            : base(key) {
            // nothing to do
        }
        ...
        public PersistentFieldForGuid(string key, Guid value)
            : this(key) {
            this.Value = value;
        }
        ...
        internal override void LoadValueFromDbDataReader(DbDataReader dataReader, int ordinal) {
            if (dataReader.IsDBNull(ordinal)) {
                this.valueFromDbDataReader = default(Guid);
            } else {
