...
namespace Framework.Persistence.Fields {
    ...
    /// </summary>
    public sealed class PersistentFieldForDateTime : PersistentFieldForStruct<DateTime> {
        ...
        public PersistentFieldForDateTime(string key)
            : base(key) {
            // nothing to do
        }
        ...
        public PersistentFieldForDateTime(string key, DateTime value)
            : this(key) {
            this.Value = value;
        }
        ...
        protected internal override string GetValueAsString() {
            return UtcDateTime.FormatAsIso8601Value(this.Value, DateTimeType.DateAndTime);
        }
        ...
        internal override void LoadValueFromDbDataReader(DbDataReader dataReader, int ordinal) {
            if (dataReader.IsDBNull(ordinal)) {
                this.valueFromDbDataReader = default(DateTime);
            } else {
