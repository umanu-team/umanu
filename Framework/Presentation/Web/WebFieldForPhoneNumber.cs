...
namespace Framework.Presentation.Web {
    ...
    /// <summary>
    /// Field control for a phone number.
    /// </summary>
    public class WebFieldForPhoneNumber : WebFieldForSingleLineText {
        ...
        /// form</param>
        public WebFieldForPhoneNumber(IPresentableFieldForElement presentableField, ViewFieldForPhoneNumber viewField, FieldRenderMode renderMode, IPresentableObject topmostParentPresentableObject, IOptionDataProvider optionDataProvider, DateTime? comparisonDate, string clientFieldIdPrefix, string clientFieldIdSuffix, PostBackState postBackState)
        ...
        /// <param name="phoneNumber">phone number to render</param>
        public static void RenderPhoneNumber(HtmlWriter html, string phoneNumber) {
            string value = System.Web.HttpUtility.HtmlEncode(phoneNumber);
            if (!string.IsNullOrEmpty(value)) {
                var attributes = new KeyValuePair<string, string>[] {
                    new KeyValuePair<string, string>("class", "tel"),
                    new KeyValuePair<string, string>("href", "tel:" + value)
                };
        ...
        protected override void RenderReadOnlyValue(HtmlWriter html) {
            if (FieldRenderMode.ListTable == this.RenderMode) {
                ...
                if (!string.IsNullOrEmpty(value)) {
                    ...
                    WebFieldForPhoneNumber.RenderPhoneNumber(html, value);
                    if (isDiffNew) {
                        html.AppendClosingTag("span");
                    }
                }
