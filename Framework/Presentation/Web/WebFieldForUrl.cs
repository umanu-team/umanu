...
namespace Framework.Presentation.Web {
    ...
    /// <summary>
    /// Field control for a url.
    /// </summary>
    public class WebFieldForUrl : WebFieldForSingleLineText {
            ...
            if (FieldRenderMode.ListTable == this.RenderMode) {
                ...
                if (this.ComparisonDate.HasValue) {
                    ...
                    if (isDiffNew && !string.IsNullOrEmpty(comparativeValue)) {
                        ...
                        WebFieldForUrl.RenderUrl(html, comparativeValue);
                ...
                if (!string.IsNullOrEmpty(value)) {
                    ...
                    WebFieldForUrl.RenderUrl(html, value);
                    if (isDiffNew) {
                        html.AppendClosingTag("span");
                    }
                }
        ...
        public static void RenderUrl(HtmlWriter html, string url) {
            string value = System.Web.HttpUtility.HtmlEncode(url);
            if (!string.IsNullOrEmpty(value)) {
                var attributes = new KeyValuePair<string, string>[] {
                    new KeyValuePair<string, string>("href", value),
                    new KeyValuePair<string, string>("rel", "noopener"),
