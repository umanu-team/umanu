...
namespace Framework.Presentation.Web {
    ...
    /// <summary>
    /// Field control for multiple e-mail addresses.
    /// </summary>
    public class WebFieldForMultipleEmailAddresses : WebFieldForMultipleSingleLineTexts {
        ...
        /// form</param>
        public WebFieldForMultipleEmailAddresses(IPresentableFieldForCollection presentableField, ViewFieldForMultipleEmailAddresses viewField, FieldRenderMode renderMode, IPresentableObject topmostParentPresentableObject, IOptionDataProvider optionDataProvider, DateTime? comparisonDate, string clientFieldIdPrefix, string clientFieldIdSuffix, PostBackState postBackState)
            ...
            if (FieldRenderMode.ListTable == this.RenderMode) {
                ...
                foreach (string currentValue in currentValues) {
                    if (!string.IsNullOrEmpty(currentValue)) {
                        ...
                        WebFieldForEmailAddress.RenderEmailAddress(html, currentValue);
                        if (!isSelectedKeyContainedInComparativeKeys) {
                            html.AppendClosingTag("span");
                        }
                    }
