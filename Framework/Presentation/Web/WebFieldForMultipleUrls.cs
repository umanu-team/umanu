...
namespace Framework.Presentation.Web {
    ...
    /// <summary>
    /// Field control for multiple URLs.
    /// </summary>
    public class WebFieldForMultipleUrls : WebFieldForMultipleSingleLineTexts {
        ...
        /// form</param>
        public WebFieldForMultipleUrls(IPresentableFieldForCollection presentableField, ViewFieldForMultipleUrls viewField, FieldRenderMode renderMode, IPresentableObject topmostParentPresentableObject, IOptionDataProvider optionDataProvider, DateTime? comparisonDate, string clientFieldIdPrefix, string clientFieldIdSuffix, PostBackState postBackState)
            ...
            if (FieldRenderMode.ListTable == this.RenderMode) {
                ...
                foreach (string currentValue in currentValues) {
                    if (!string.IsNullOrEmpty(currentValue)) {
                        ...
                        WebFieldForUrl.RenderUrl(html, currentValue);
                        if (!isSelectedKeyContainedInComparativeKeys) {
                            html.AppendClosingTag("span");
                        }
                    }
